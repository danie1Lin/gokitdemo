...
func MakeHTTPHandler(endpoints Endpoints, responseEncoder httptransport.EncodeResponseFunc, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("GET").Path("/events/").Handler(httptransport.NewServer(
		endpoints.CreateEventEndpoint,
		DecodeHTTPCreateEventZeroRequest,
		responseEncoder,
		serverOptions...,
	))
	m.Methods("POST").Path("/events").Handler(httptransport.NewServer(
		endpoints.CreateEventEndpoint,
		DecodeHTTPCreateEventOneRequest,
		responseEncoder,
		serverOptions...,
	))
	return m
...
// DecodeHTTPCreateEventZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded createevent request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPCreateEventZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	...
	if EventCreateEventStrArr, ok := queryParams["Event"]; ok {
		EventCreateEventStr := EventCreateEventStrArr[0]

		err = json.Unmarshal([]byte(EventCreateEventStr), req.Event)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode EventCreateEvent from %v", EventCreateEventStr)
		}
...
// DecodeHTTPCreateEventOneRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded createevent request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPCreateEventOneRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.CreateEventRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
